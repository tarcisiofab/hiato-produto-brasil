...
> **Reprodutibilidade total:** todos os dados são baixados automaticamente por código (APIs do BCB/SGS, FTP do IBGE e Ipeadata). Nenhum arquivo manual é necessário. `Rscript R/hiato_completo.R` reproduz cada número e figura do trabalho em ~2–3 minutos.

---

## Resultados em uma figura

![Síntese](output/g11_sintese.png)
...
├── output/                     # Figuras g1–g11 e console da rodada de referência
├── sessionInfo.txt             # Versões de R e pacotes da rodada de referência
└── README.md
```
...
Com β₁=0,6 e β₂=0,4: **0,658·FP + 0,342·HP**. Enunciado, demonstração e corolários em [`docs/secao_proposicao_areosa.md`](docs/secao_proposicao_areosa.md); verificação numérica (erro máximo 4,6×10⁻¹¹ ante o sistema exato 2T×2T) em [`R/verif_areosa.R`](R/verif_areosa.R).
...
```bash
Rscript R/hiato_completo.R      # ~2-3 min; gera g1–g11.png no diretório de execução
Rscript R/verif_areosa.R        # verificação da Proposição 1 (~10 s)
```
...
- Os dados das APIs são **vintage-dependentes**: rodadas em datas diferentes podem produzir números ligeiramente distintos dos reportados (revisões do PIB e atualizações das séries). A rodada de referência está congelada em `output/console_output.txt`.
